...
-- 3. Get all topics where etag exists
...
set sql_mode='';
select count(*),errors.*,topics.* from errors left join topics on ref_X_sha256=t_sha256 where t_type="ERR" group by t_sha256,e_content order by t_sha256; -- add where c_added > date
select e_reftype,e_content,count(distinct t_url) from errors left join topics on ref_X_sha256=t_sha256 group by e_content order by e_content; -- add where c_added > date
